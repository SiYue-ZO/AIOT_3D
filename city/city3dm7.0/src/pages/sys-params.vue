  ...
  <el-card>
    ...
    <div v-show="isTableShow" class="my-card-body" :style="{height: cardBodyHeight}">
      <el-table class="result-table" :data="tableDate.slice((currentPage-1)*pagesize,currentPage*pagesize)"
                ...
                :default-sort="{prop: 'sysName', order: 'ascending'}" @row-click="rowClick">
        <el-table-column label="参数名称" prop="sysName" sortable></el-table-column>
        <el-table-column label="类型" prop="sysType" sortable></el-table-column>
        <el-table-column label="内容" prop="sysContent" sortable></el-table-column>
      </el-table>
      <el-pagination
        class="my-pagination"
    ...
    <div v-show="!isTableShow" class="my-card-body" :style="{height: cardBodyHeight}">
      <el-form ref="formData" :model="formData" :rules="rules" label-width="90px" class="my-form">
        <el-form-item label="名称" prop="sysName">
          <el-input v-model="formData.sysName" placeholder="请填写参数名称"></el-input>
        </el-form-item>
        <el-form-item label="类型" prop="sysType">
          <el-input v-model="formData.sysType" placeholder="请填写参数所属类型"></el-input>
        </el-form-item>
        <el-form-item label="内容" prop="sysContent">
          <el-input v-model="formData.sysContent" placeholder="请填写参数的具体内容"></el-input>
        </el-form-item>
      </el-form>
    </div>
  </el-card>
...
<script>
  import {getSys, deleteSys, insertSys, updateSys} from "../api/api";
    ...
    data() {
      return {
        ...
        rules: {
          ...
          sysType: [{required: true, message: '请输入', trigger: 'blur'}],
          sysContent: [{required: true, message: '请输入', trigger: 'blur'}],
        },
        formData: {
          sysId: '',
          sysName: '',
      ...
      deleteData() {
        if (this.currentId !== '') {
          this.$confirm('删除后无法恢复，请确认是否删除？', '删除数据', {
            ...
            let params = {
              sysId: this.currentId
            };
            deleteSys(params).then(res => {
              if (res.data === 1) {
                this.$message({
                  type: 'success',
                  message: '删除成功，刷新列表！'
          ...
          if (valid) {
            if (this.isEdit) {
              this.$confirm('请确认信息填写无误，是否保存？', '确认信息', {
                ...
                updateSys(this.formData).then(res => {
                  if (res.data === 1) {
                    this.$message({
                      type: 'success',
                      message: '修改成功，返回列表！'
              ...
              this.$confirm('请确认信息填写无误，是否保存？', '确认信息', {
                ...
                insertSys(this.formData).then(res => {
                  if (res.data === 1) {
                    this.$message({
                      type: 'success',
                      message: '新增成功，返回列表！'
    ...
    watch: {
      devFilter: function (val, oldVal) {
        this.tableDate = this.queryResult.filter(
          item => (~item.sysName.indexOf(val)
            || ~item.sysType.indexOf(val)
            || ~item.sysContent.indexOf(val))
        );
      }
    },
    mounted() {
