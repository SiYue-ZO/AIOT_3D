  ...
  <el-card>
    ...
    <div v-show="isTableShow" class="my-card-body" :style="{height: cardBodyHeight}">
      <el-table class="result-table" :data="tableDate.slice((currentPage-1)*pagesize,currentPage*pagesize)"
                ...
                @selection-change="handleSelectionChange" ref="multipleTable" row-key="accessManId"
        ...
        <el-table-column label="门禁相机" prop="cameraId" sortable></el-table-column>
        <el-table-column label="人员身份" prop="userId" sortable></el-table-column>
    ...
    <div v-show="!isTableShow" class="my-card-body" :style="{height: cardBodyHeight}">
      <el-form ref="formData" :model="formData" :rules="rules" label-width="90px" class="my-form">
        <el-form-item label="门禁相机" prop="cameraId">
        ...
        <el-form-item label="人员类型" prop="accessType">
          <el-select v-model="formData.accessType" placeholder="" readonly>
            ...
            <el-option label="陌生人" value="2"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="人员身份" prop="userId">
...
<script>
  import {deleteAccessMan, getAccessMan, batchDeleteAccessMan} from "../api/api";
  ...
  export default {
    ...
    methods: {
      ...
      rowClick(row, column, event) {
        this.currentId = row.accessManId;
        this.currentRow = row;
      },
      ...
      deleteData() {
        if (this.currentId !== '') {
          this.$confirm('删除后无法恢复，请确认是否删除？', '删除数据', {
            ...
            let params = {
              accessManId: this.currentId
            };
            deleteAccessMan(params).then(res => {
              if (res.data === 1) {
                this.$message({
                  type: 'success',
                  message: '删除成功，刷新列表！'
      ...
      batchDeleteData() {
        if (this.multipleSelection !== []) {
          this.$confirm("删除后无法恢复，请确认是否删除？", "删除数据", {
            ...
            batchDeleteAccessMan(this.multipleSelection).then((res) => {
              if (res.status === 200) {
                this.$message({
                  message: '删除成功，刷新列表！',
                  type: 'success'
