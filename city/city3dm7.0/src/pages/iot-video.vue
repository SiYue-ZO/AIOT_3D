        ...
        <el-form ref="formData" :model="formData" :rules="rules" label-width="90px">
          ...
          <el-form-item label="方位角" prop="camHeading" v-show="formIsNotNode">
            <el-slider v-model="formData.camHeading" style="width: 90%" show-input :min="0" :max="360"
                       :step="0.1" @input="updateCamerVideo"></el-slider>
          </el-form-item>
          ...
          <el-form-item label="裁切范围" prop="camHoles" v-show="formIsNotNode">
            <el-input v-model="formData.camHoles" placeholder="请创建裁切范围" style="width: 90%"></el-input>
          </el-form-item>
          <el-form-item label="裁切画板" prop="camHoles" v-show="formIsNotNode">
            <canvas id="vscanvas" width="490px" height="276px"></canvas>
            <font-awesome-icon class="form-fai vscanvas-fai" :icon="['fas', 'expand']" fixed-width
                               title="生成快照，开始绘制"
                               @click="openHole" style="top:0"></font-awesome-icon>
            <font-awesome-icon class="form-fai vscanvas-fai" :icon="['fas', 'cut']" fixed-width title="完成绘制"
                               @click="drawPolyline" style="top:40px;"></font-awesome-icon>
            <font-awesome-icon class="form-fai vscanvas-fai" :icon="['fas', 'trash-alt']" fixed-width title="清除绘制"
                               @click="clearRect" style="top:80px;"></font-awesome-icon>
            <font-awesome-icon class="form-fai vscanvas-fai" :icon="['fas', 'check-circle']" fixed-width
                               title="完成裁切"
                               @click="overRect" style="top:120px;"></font-awesome-icon>
            <div id="imgdiv"></div>
          </el-form-item>
        </el-form>
      </div>
...
export default {
  ...
  methods: {
    ...
    createCameraVideo(node, videoElement) {
      ...
      // console.log(cameraVideo._primitive.appearance.material);
      // cameraVideo._primitive.appearance.material.translucent = true;
      // cameraVideo._primitive.appearance.material.uniforms.repeat = new Cesium.Cartesian2(3.0, 3.0);
      // 2.2.2.5 加入到场景中去
    ...
    updateCamerVideo() {
      ...
      if (cameraVideo) {
        let heading = Cesium.Math.toRadians(+this.formData.camHeading);
        let pitch = Cesium.Math.toRadians(+this.formData.camPitch);
        let roll = Cesium.Math.toRadians(+this.formData.camRoll);
        let poiArr = this.formData.camPosition.split(',');
        let positionCt3 = Cesium.Cartesian3.fromDegrees(poiArr[0], poiArr[1], poiArr[2]);
        let ivm = this.getInverseViewMatrix(positionCt3, heading, pitch, roll);
        cameraVideo.inverseViewMatrix = ivm;
      }
    },
    updateCamFov() {
      let cameraVideo = this.getcfgVideo();
      if (cameraVideo) {
        cameraVideo.frustum.fov = Cesium.Math.toRadians(this.formData.camFov);
      }
    },
    updateCamAspectRatio() {
      let cameraVideo = this.getcfgVideo();
      if (cameraVideo) {
        cameraVideo.frustum.aspectRatio = +this.formData.camAspectratio;
      }
    },
    updateCamNear() {
      let cameraVideo = this.getcfgVideo();
      if (cameraVideo) {
        cameraVideo.frustum.near = +this.formData.camNear;
      }
    },
    updateCamFar() {
      let cameraVideo = this.getcfgVideo();
      if (cameraVideo) {
        cameraVideo.frustum.far = +this.formData.camFar;
      }
    },
    getcfgVideo() {
      let cameraVideo;
    ...
    getInverseViewMatrix(position, heading, pitch, roll, result) {
      let obj = {};
      obj.position = position;
      obj.heading = heading;
      obj.pitch = pitch;
      obj.roll = roll;
      let inverseViewMatrix = this.hpr2m(obj, result);
      return inverseViewMatrix;
    },
    openHole() {
      if (document.getElementById("videohole") === null) {
        videoHoleElement = document.createElement('video');
        videoHoleElement.id = 'videohole';
        if (this.isLocalOrRoam) {
          videoHoleElement.src = this.formData.camUrlHttp + '://' + this.formData.camUrlIp + ':' + this.formData.camUrlPost;
        } else {
          videoHoleElement.src = common.serverIp + 'city3dfile/' + this.formData.camUrl;
        }
        videoHoleElement.style.position = 'absolute';
        videoHoleElement.style.zIndex = '-100';
        videoHoleElement.style.visibility = 'hidden';
        videoHoleElement.crossOrigin = 'anonymous';
        videoHoleElement.autoplay = true;
        videoHoleElement.loop = true;
        videoHoleElement.muted = true;
        document.getElementById("videoSource").appendChild(videoHoleElement);
      }

      vscanvas = document.getElementById("vscanvas");
      cxt = vscanvas.getContext("2d");
      setTimeout(() => {
        cxt.drawImage(videoHoleElement, 0, 0, vscanvas.width, vscanvas.height);
      }, 100);
      vscanvas.onmousedown = function (e) {
    ...
    drawPolyline() {
      ...
      cxt.fillRect(0, 0, vscanvas.width, vscanvas.height);
      cxt.beginPath();
      for (let i = 0; i < lineArr.length; i++) {
        cxt.lineTo(lineArr[i].x, lineArr[i].y);
      }
      cxt.closePath();
      cxt.fillStyle = "rgb(255,255,255)";
      cxt.fill();
      lineArr = [];
      if (imgElement && document.getElementById("imgdiv").childElementCount !== 0) {
        document.getElementById("imgdiv").removeChild(imgElement);
        imgElement = undefined;
      }
      imgElement = document.createElement('img');
      imgElement.src = vscanvas.toDataURL("image/jpeg");
      document.getElementById("imgdiv").appendChild(imgElement);
    },
    clearRect() {
      cxt.clearRect(0, 0, vscanvas.width, vscanvas.height);
      if (document.getElementById("videohole") !== null) {
        this.destroyVideoElement(videoHoleElement);
      }
      if (imgElement && document.getElementById("imgdiv").childElementCount !== 0) {
        document.getElementById("imgdiv").removeChild(imgElement);
        imgElement = undefined;
    ...
    overRect() {
      if (imgElement) {
        this.formData.camHoles = imgElement.src;
        this.clearRect();
      } else {
        this.$message({
          message: '请先确认剪切范围！',
