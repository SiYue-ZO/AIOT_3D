  ...
  <div>
    <el-card v-show="isTableShow">
      ...
      <div class="my-card-body" :style="{height: cardBodyHeight}">
        <el-table class="result-table" :data="tableDate.slice((currentPage-1)*pagesize,currentPage*pagesize)"
                  ...
                  @selection-change="handleSelectionChange" ref="multipleTable" row-key="machineId"
                  :default-sort="{prop: 'machineType', order: 'ascending'}" @row-click="rowClick">
          <el-table-column type="selection" :reserve-selection="true" width="42"></el-table-column>
          <el-table-column label="类型" prop="machineType" sortable></el-table-column>
    ...
    <div v-show="!isTableShow">
        ...
        <div class="my-card-body" :style="{height: cardBodyHeight}">
          <el-form ref="formData" :model="formData" :rules="rules" label-width="90px">
            ...
            <div class="row_display">
              ...
              <el-form-item label="类型" prop="machineType" style="margin-left:15px;">
                <el-select v-model="formData.machineType" placeholder="请选择" style="width: 93%">
                  <el-option label="电焊机" value="电焊机"></el-option>
                  <el-option label="切割机" value="切割机"></el-option>
                  <el-option label="砂轮机" value="砂轮机"></el-option>
                  <el-option label="电钻机" value="电钻机"></el-option>
                  <el-option label="电锯" value="电锯"></el-option>
                  <el-option label="射钉枪" value="射钉枪"></el-option>
                  <el-option label="其它" value="其它"></el-option>
                </el-select>
              </el-form-item>
            </div>
            <el-form-item label="备注" prop="machineRemark">
              <el-input type="textarea" v-model="formData.machineRemark" placeholder="请填写"
                        :autosize="{minRows: 2, maxRows: 10}" style="width: 70%"></el-input>
            </el-form-item>
            <el-form-item label="机械照片" prop="machineImgUrl">
      ...
      <el-card id="right-card" class="right-card" :style="{width: rightWidth, height: viewerHeight}">
        <div class="row_display">
          <div class="right-card-title">机械所有人</div>
    ...
    data() {
      return {
        ...
        rules: {
          ...
          machineType: [{required: true, message: '请输入', trigger: 'blur'}],
        },
        formData: {
          machineId: '',
          ppId: '',
    ...
    methods: {
      ...
      editData() {
        if (this.currentId !== '') {
          ...
          this.imagePreview = common.serverIp + 'city3dfile/' + this.formData.machineImgUrl;
      ...
      deleteData() {
        if (this.currentId !== '') {
          this.$confirm('删除后无法恢复，请确认是否删除？', '删除数据', {
            ...
            let params = {
              machineId: this.currentId,
              machineImgUrl: this.currentRow.machineImgUrl
            };
            deletePeopleMachine(params).then(res => {
              if (res.data === 1) {
                this.$message({
                  type: 'success',
                  message: '删除成功，刷新列表！'
      ...
      batchDeleteData() {
        if (this.multipleSelection !== []) {
          this.$confirm("删除后无法恢复，请确认是否删除？", "删除数据", {
            ...
            batchDeletePeopleMachine(this.multipleSelection).then((res) => {
              if (res.status === 200) {
                this.$message({
                  message: '删除成功，刷新列表！',
                  type: 'success'
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            formData.append('machineType', params.machineType);
            formData.append('machineImgUrl', params.machineImgUrl);
            formData.append('machineRemark', params.machineRemark);
            if (this.isEdit) {
              this.$confirm('请确认信息填写无误，是否保存？', '确认信息', {
                ...
                updatePeopleMachine(formData).then(res => {
                  if (res.data === 1) {
                    this.$message({
                      type: 'success',
                      message: '修改成功，返回列表！'
              ...
              this.$confirm('请确认信息填写无误，是否保存？', '确认信息', {
                ...
                insertPeopleMachine(formData).then(res => {
                  if (res.data === 1) {
                    this.$message({
                      type: 'success',
                      message: '新增成功，返回列表！'
    ...
    watch: {
      devFilter: function (val, oldVal) {
        this.tableDate = this.queryResult.filter(
          item => (~item.machineType.indexOf(val)
