  ...
  <el-card>
    ...
    <div v-show="isTableShow" class="my-card-body" :style="{height: cardBodyHeight}">
      <el-table class="result-table" :data="tableDate.slice((currentPage-1)*pagesize,currentPage*pagesize)"
                ...
                @selection-change="handleSelectionChange" ref="multipleTable" row-key="faceId"
                :default-sort="{prop: 'captureTime', order: 'descending'}" @row-click="rowClick">
        <el-table-column type="selection" :reserve-selection="true" width="42"></el-table-column>
        <el-table-column label="报警相机" prop="cameraId" sortable></el-table-column>
        <el-table-column label="身份证号" prop="cardId" sortable></el-table-column>
        <el-table-column label="抓拍类型" prop="repoType" sortable></el-table-column>
        <el-table-column label="报警时间" prop="captureTime" sortable></el-table-column>
    ...
    <div v-show="!isTableShow" class="my-card-body" :style="{height: cardBodyHeight}">
      <el-form ref="formData" :model="formData" :rules="rules" label-width="90px" class="my-form">
        <el-form-item label="报警相机" prop="cameraId">
        ...
        <el-form-item label="报警时间" prop="captureTime">
          <el-input v-model="formData.captureTime" placeholder="" readonly></el-input>
        </el-form-item>
        <el-form-item label="抓拍类型" prop="repoType">
          <el-select v-model="formData.repoType" placeholder="请选择">
        ...
        <el-form-item label="身份证号" prop="cardId">
          <el-input v-model="formData.cardId" placeholder=""></el-input>
        </el-form-item>
        <el-form-item label="姓名" prop="name">
          <el-input v-model="formData.name" placeholder=""></el-input>
        </el-form-item>
        <el-form-item label="报警描述" prop="description">
          <el-input v-model="formData.description" placeholder="" type="textarea" :row="3"></el-input>
        </el-form-item>
      </el-form>
    </div>
  </el-card>
...
<script>
  import {deleteFaceIdentity, getFaceIdentity, updateFaceIdentity, batchDeleteFaceIdentity} from "../api/api";
    ...
    data() {
      return {
        ...
        rules: {
          ...
          cardId: [{required: true, message: '请输入', trigger: 'blur'}],
          captureTime: [{required: true, message: '请输入', trigger: 'blur'}],
        },
        formData: {
          faceId: '',
          serialNumber: '',
      ...
      deleteData() {
        if (this.currentId !== '') {
          this.$confirm('删除后无法恢复，请确认是否删除？', '删除数据', {
            ...
            let params = {
              faceId: this.currentId
            };
            deleteFaceIdentity(params).then(res => {
              if (res.data === 1) {
                this.$message({
                  type: 'success',
                  message: '删除成功，刷新列表！'
      ...
      batchDeleteData() {
        if (this.multipleSelection !== []) {
          this.$confirm("删除后无法恢复，请确认是否删除？", "删除数据", {
            ...
            batchDeleteFaceIdentity(this.multipleSelection).then((res) => {
              if (res.status === 200) {
                this.$message({
                  message: '删除成功，刷新列表！',
                  type: 'success'
          ...
          if (valid) {
            if (this.isEdit) {
              this.$confirm('请确认信息填写无误，是否保存？', '确认信息', {
                ...
                updateFaceIdentity(this.formData).then(res => {
                  if (res.data === 1) {
                    this.$message({
                      type: 'success',
                      message: '修改成功，返回列表！'
      ...
      devFilter: function (val, oldVal) {
        this.tableDate = this.queryResult.filter(
          item => (~item.cameraId.indexOf(val)
            || ~item.cardId.indexOf(val)
            || ~item.name.indexOf(val)
            || ~item.serialNumber.indexOf(val)
            || ~item.captureTime.indexOf(val)
          ));
      }
    },
    mounted() {
