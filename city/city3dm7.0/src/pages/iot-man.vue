    ...
    <el-card v-show="isTableShow">
      ...
      <div class="my-card-body" :style="{height: cardBodyHeight}">
        <el-table class="result-table" :data="tableDate.slice((currentPage-1)*pagesize,currentPage*pagesize)"
                  ...
                  :default-sort="{prop: 'origin', order: 'ascending'}" @row-click="rowClick">
          <el-table-column label="姓名" prop="name" sortable></el-table-column>
          <el-table-column label="职务" prop="post" sortable></el-table-column>
          <el-table-column label="警务类型" prop="origin" sortable></el-table-column>
          <el-table-column label="设备类型" prop="deviceType" sortable></el-table-column>
          <el-table-column label="设备号码" prop="deviceNum" sortable></el-table-column>
        </el-table>
        <el-pagination
          class="my-pagination"
        ...
        <div class="my-card-body" :style="{height: cardBodyHeight}">
          <el-form ref="formData" :model="formData" :rules="rules" label-width="90px">
            ...
            <div class="row_display">
              ...
              <el-form-item label="警务类型" prop="origin" style="margin-left:15px;">
                <el-select v-model="formData.origin" placeholder="请选择职务" style="width: 93%">
                  <el-option label="特警" value="特警"></el-option>
                  <el-option label="巡警" value="巡警"></el-option>
                  <el-option label="治安警察" value="治安警察"></el-option>
                </el-select>
              </el-form-item>
            </div>
            <div class="row_display">
              <el-form-item label="职务" prop="post">
                <el-input v-model="formData.post" placeholder="请填写职务"></el-input>
              </el-form-item>
              <el-form-item label="警衔" prop="policeRank" style="margin-left:15px;">
                <el-input v-model="formData.policeRank" placeholder="请填写警衔"></el-input>
              </el-form-item>
            </div>
            <div class="row_display">
              <el-form-item label="设备号码" prop="deviceNum">
                <el-input v-model="formData.deviceNum" placeholder="请填写外勤设备号码"></el-input>
              </el-form-item>
              <el-form-item label="设备类型" prop="deviceType" style="margin-left:15px;">
                <el-select v-model="formData.deviceType" placeholder="请选择设备类型" style="width: 93%">
                  <el-option label="对讲机" value="对讲机"></el-option>
                  <el-option label="执法仪" value="执法仪"></el-option>
                  <el-option label="4G执法仪" value="4G执法仪"></el-option>
                </el-select>
              </el-form-item>
            </div>
            <div class="row_display">
              <el-form-item label="身份证号" prop="identity">
                <el-input v-model="formData.identity" placeholder="请填写人员身份证号"></el-input>
              </el-form-item>
              <el-form-item label="性别" prop="gender" style="margin-left:15px;">
            ...
            <div class="row_display">
              <el-form-item label="民族" prop="nation">
                <el-input v-model="formData.nation" placeholder="请填写人员民族"></el-input>
              </el-form-item>
              <el-form-item label="AI编号" prop="birth" style="margin-left:15px;">
                <el-input v-model="formData.birth" placeholder="请填写AI白名单中对应的编号"></el-input>
              </el-form-item>
            </div>
            <div class="row_display">
              <el-form-item label="入职时间" prop="jobDate">
                <el-date-picker v-model="formData.jobDate" type="date" placeholder="请选择日期"
                                style="width: 87%"></el-date-picker>
              </el-form-item>
              <el-form-item label="学历" prop="education" style="margin-left:-15px;">
                <el-input v-model="formData.education" placeholder="请填写人员民族"></el-input>
              </el-form-item>
            </div>
          </el-form>
        </div>
...
<script>
import {
  deletePolice, getAllandPoliceById,
  getPolice,
  getPoliceAndEquipmentsByPuid,
  insertPolice,
  ...
  methods: {
    ...
    editData() {
      if (this.currentId !== '') {
        ...
        let params = {
          deviceNum: this.currentRow.deviceNum
        };
        getPoliceAndEquipmentsByPuid(params).then(res => {
          let policeEquipments = res.data.policeEquipments;
          policeEquipments.map(item => {
    ...
    deleteData() {
      if (this.currentId !== '') {
        this.$confirm('删除后无法恢复，请确认是否删除？', '删除数据', {
          ...
          let params = {
            policeId: this.currentId,
            image: this.currentRow.image
          };
          deletePolice(params).then(res => {
            if (res.data === 1) {
              this.$message({
                type: 'success',
                message: '删除成功，刷新列表！'
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          params.birth = new Date(params.birth).toLocaleDateString().replace(/\//g, '-');
          params.jobDate = new Date(params.jobDate).toLocaleDateString().replace(/\//g, '-');
          let formData = new FormData();
          this.fileList.map(item => {
            formData.append('files', item.raw);
          });
          formData.append('policeId', params.policeId);
          ...
          formData.append('jobDate', params.jobDate);
          formData.append('education', params.education);
          formData.append('origin', params.origin);
          formData.append('post', params.post);
          formData.append('policeRank', params.policeRank);
          formData.append('deviceNum', params.deviceNum);
          ...
          if (this.isEdit) {
            this.$confirm('请确认信息填写无误，是否保存？', '确认信息', {
              ...
              updatePolice(formData).then(res => {
                if (res.data === 1) {
                  this.$message({
                    type: 'success',
                    message: '修改成功，返回列表！'
            ...
            this.$confirm('请确认信息填写无误，是否保存？', '确认信息', {
              ...
              insertPolice(formData).then(res => {
                if (res.data === 1) {
                  this.$message({
                    type: 'success',
                    message: '新增成功，返回列表！'
    ...
    devFilter: function (val, oldVal) {
      this.tableDate = this.queryResult.filter(
        item => (~item.name.indexOf(val)
          ...
          || ~item.deviceType.indexOf(val)
          || ~item.deviceNum.indexOf(val)
        ));
    }
  },
  mounted() {
