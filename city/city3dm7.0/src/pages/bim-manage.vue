    ...
    <el-card id="left-card" class="left-card" :style="{width: leftWidth, height: viewerHeight}">
      ...
      <div v-show="isTableShow" class="my-card-body" :style="{height: cardBodyHeight}">
        <el-table class="result-table" :data="tableDate.slice((currentPage-1)*pagesize,currentPage*pagesize)"
                  ...
                  @selection-change="handleSelectionChange" ref="multipleTable" row-key="tilesId"
...
import {deleteBim, getBim, insertBim, updateBim, batchDeleteBim, getAllandBimById} from "../api/api";
    ...
    getBimData() {
      ...
      if (localStorage.getItem("roleId") === "sunhengyi18826126699") {
        ...
        getAllandBimById(params).then(res => {
          let resultArr = [];
          res.data.sysRoleBim.map(item => {
            resultArr.push(item.bim);
          });
    ...
    batchDeleteData() {
      if (this.multipleSelection !== []) {
        this.$confirm("删除后无法恢复，请确认是否删除？", "删除数据", {
          ...
          batchDeleteBim(this.multipleSelection).then((res) => {
            if (res.status === 200) {
              this.$message({
                message: '删除成功，刷新列表！',
                type: 'success'
        ...
        if (valid) {
          ...
          if (this.isEdit) {
            this.$confirm('请确认信息填写无误，是否保存？', '确认信息', {
              ...
              updateBim(this.formData).then(res => {
                if (res.data === 1) {
                  this.$message({
                    type: 'success',
                    message: '修改成功，返回列表！'
