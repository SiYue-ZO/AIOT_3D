    ...
    <el-card id="left-card" class="left-card" :style="{width: leftWidth, height: viewerHeight}">
      ...
      <div v-show="isTableShow" class="my-card-body" :style="{height: cardBodyHeight}">
        <el-table class="result-table" :data="tableDate.slice((currentPage-1)*pagesize,currentPage*pagesize)"
                  ...
                  :default-sort="{prop: 'pType', order: 'ascending'}" @row-click="rowClick">
          <el-table-column type="selection" :reserve-selection="true" width="42"></el-table-column>
          <el-table-column label="点位名称" prop="pName" sortable></el-table-column>
          <el-table-column label="类别" prop="pType" sortable></el-table-column>
        </el-table>
        <el-pagination
          class="my-pagination"
      ...
      <div v-show="!isTableShow" class="my-card-body" :style="{height: cardBodyHeight}">
        <el-form ref="formData" :model="formData" :rules="rules" label-width="90px">
          <el-form-item label="点位名称" prop="pName">
            <el-input v-model="formData.pName" placeholder="请填写点位名称" style="width: 90%"></el-input>
          </el-form-item>
          <el-form-item label="类别" prop="pType">
            <el-input v-model="formData.pType" placeholder="请填写点位类别" style="width: 90%"></el-input>
          </el-form-item>
          <el-form-item label="点位坐标" prop="pPoi">
            <el-input v-model="formData.pPoi" placeholder="请左键地图点选、右键结束或填写点位坐标值"
          ...
          <el-form-item label="颜色" prop="pColor">
            <el-input v-model="formData.pColor" placeholder="请选择或填入CSS颜色值" style="width: 90%"></el-input>
            <el-color-picker class="color-picker" v-model="formData.pColor" size="small"
                             :predefine="predefineColors"></el-color-picker>
          </el-form-item>
          <el-form-item label="登记时间" prop="pTime">
            <el-date-picker v-model="formData.pTime" align="left" type="date"
                            placeholder="选择日期" :picker-options="pickerOptions" style="width: 90%">
            </el-date-picker>
          </el-form-item>
          <el-form-item label="确症人员" prop="nameChecked">
            <el-input v-model="formData.nameChecked" placeholder="请填写" style="width: 90%"></el-input>
          </el-form-item>
          <el-form-item label="身份证" prop="idChecked">
            <el-input v-model="formData.idChecked" placeholder="请填写" style="width: 90%"></el-input>
          </el-form-item>
          <el-form-item label="密接人员" prop="nameClose">
            <el-input v-model="formData.nameClose" placeholder="请填写" style="width: 90%"></el-input>
          </el-form-item>
          <el-form-item label="身份证" prop="idClose">
            <el-input v-model="formData.idClose" placeholder="请填写" style="width: 90%"></el-input>
          </el-form-item>
          <el-form-item label="次密接人员" prop="nameSubClose">
            <el-input v-model="formData.nameSubClose" placeholder="请填写" style="width: 90%"></el-input>
          </el-form-item>
          <el-form-item label="身份证" prop="idSubClose">
            <el-input v-model="formData.idSubClose" placeholder="请填写" style="width: 90%"></el-input>
          </el-form-item>
          <el-form-item label="活动范围" prop="pLiving">
            <el-input v-model="formData.pLiving" placeholder="请填写" style="width: 90%" type="textarea"
                      :autosize="{minRows: 2, maxRows: 2}"></el-input>
          </el-form-item>
          <el-form-item label="流调情况" prop="pFlow">
            <el-input v-model="formData.pFlow" placeholder="请填写" style="width: 90%" type="textarea"
                      :autosize="{minRows: 2, maxRows: 2}"></el-input>
          </el-form-item>
          <el-form-item label="备注" prop="pNote">
            <el-input v-model="formData.pNote" placeholder="请填写" style="width: 90%" type="textarea"
                      :autosize="{minRows: 2, maxRows: 2}"></el-input>
          </el-form-item>
        </el-form>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        pType: [{required: true, message: '请输入', trigger: 'blur'}],
        pPoi: [{required: true, message: '请输入', trigger: 'blur'}],
  ...
  methods: {
    ...
    addPoiToMap(row) {
      viewer.entities.removeAll();
      let pois = stringToNumber(row.pPoi);
    ...
    deleteData() {
      if (this.currentId !== '') {
        this.$confirm('删除后无法恢复，请确认是否删除？', '删除数据', {
          ...
          deletePolicePoint(params).then(res => {
            if (res.data === 1) {
              this.$message({
                type: 'success',
                message: '删除成功，刷新列表！'
    ...
    batchDeleteData() {
      if (this.multipleSelection !== []) {
        this.$confirm("删除后无法恢复，请确认是否删除？", "删除数据", {
          ...
          batchDeletePolicePoint(this.multipleSelection).then((res) => {
            if (res.status === 200) {
              this.$message({
                message: '删除成功，刷新列表！',
                type: 'success'
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.formData.pTime = new Date(this.formData.pTime).toLocaleDateString().replace(/\//g, '-');
          if (this.isEdit) {
            this.$confirm('请确认信息填写无误，是否保存？', '确认信息', {
              ...
              updatePolicePoint(this.formData).then(res => {
                if (res.data === 1) {
                  this.$message({
                    type: 'success',
                    message: '修改成功，返回列表！'
            ...
            this.$confirm('请确认信息填写无误，是否保存？', '确认信息', {
              ...
              insertPolicePoint(this.formData).then(res => {
                if (res.data === 1) {
                  this.$message({
                    type: 'success',
                    message: '新增成功，返回列表！'
  ...
  watch: {
    devFilter: function (val, oldVal) {
      this.tableDate = this.queryResult.filter(
        item => (~item.pName.indexOf(val)
          || ~item.pType.indexOf(val)
        ));
    }
  },
  mounted() {
