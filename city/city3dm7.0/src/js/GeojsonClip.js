...
export const geoUtil = {
  ...
  polygonClipByLine: function (polygon, clipLine) {
    if (polygon.geometry.type === 'Polygon') {
      ...
      polygons.forEach(function (polygon, index) {
        ...
        if (turf.lineIntersect(polyLine, clipLine).features.length === 2) {
          if (!clipPolygon) {
            ...
            return {state: '裁剪失败', message: 'MultiPolygon只能有一个多边形与切割线存在交点'};
          }
        }
      });
      if (clipPolygonIndex !== -1) {
        ...
        return {state: '裁剪失败', message: 'MultiPolygon与切割线无交点'};
      }
    } else {
      return {state: '裁剪失败', message: '输入的多边形类型为错误'};
    }
  },
  ...
  _singlePolygonClip: function (polyLine, clipLine) {
    ...
    if (intersects.features.length !== 2) {
      return {state: '裁剪失败', message: '切割线与多边形交点应该为2个,当前交点个数为' + intersects.features.length};
    }
    ...
    if (turf.booleanPointInPolygon(clipLineStartPoint, polygon) || turf.booleanPointInPolygon(clipLineEndPoint, polygon)) {
      return {state: '裁剪失败', message: '切割线起点或终点不能在 裁剪多边形内部'};
    }
    ...
    let slicedPolyLine_2 = turf.lineString(pointList);
  ...
  _multiPolygonClip: function (polyLine, clipLine) {
    ...
    for (let i = 0; i < polyLine.geometry.coordinates.length; i++) {
      ...
      if (turf.booleanClockwise(splitPolyline)) {
        if (outPolyline) {
          return {state: '裁剪失败', message: '出现了两个外部多边形无法处理'};
        } else {
        ...
        if (intersects.features.length > 0) {
          return {state: '裁剪失败', message: '切割线不能与内环有交点'};
        }
        insidePolylineList.push(splitPolyline)
      }
    }
    let resultCollection = this._singlePolygonClip(outPolyline, clipLine);
