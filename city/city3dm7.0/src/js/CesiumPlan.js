(function (window) {
  ...
  function define_CesiumPlan() {
      ...
      if (isRightOver) {
        ...
        drawHandler.setInputAction(function () {
          ...
          let ePlanConStr = '{'
            ...
            + '\"wallImg\":\"' + options.polylinecolor.image._value + '\",'
            + '\"poi\":\"' + tempPointsStr + '\"'
            + '};';
          ePlanCon.push(ePlanConStr);
          ...
          let repeat;
          let wallImgMaterial = options.polylinecolor;
          if (options.wallType === 'wall7') {
            repeat = new Cesium.Cartesian2(measureResult / 6, 1.0);
            wallImgMaterial.image._value = common.serverIp + 'city3dfile/' + options.polylinecolor.image._value;
    ...
    function ComputeRoamingLineProperty(viewer, Lines, time, isRe) {
      ...
      //roamStop = Cesium.JulianDate.addSeconds(roamStart, time, new Cesium.JulianDate());
      ...
      if (isRightOver) {
        ...
        roamHandler.setInputAction(function () {
          ...
          for (let i = 0; i < measureLinePositonsArray.length; i++) {
            ...
            let longitudeString = Cesium.Math.toDegrees(cartographic.longitude);
            let latitudeString = Cesium.Math.toDegrees(cartographic.latitude);
            let heightString = cartographic.height + Number(options.roamHeight);
            tempPointsStr += longitudeString + "," + latitudeString + "," + heightString + ",";
          }
          roamLine = tempPointsStr.substring(0, tempPointsStr.length - 1);
          isRightOver = true;
        }, Cesium.ScreenSpaceEventType.RIGHT_CLICK);
      } else {
        alert('请先结束前一绘制操作！');
    ...
    CesiumPlan.clearEplanCon = function () {
      ePlanCon = [];
    };

    CesiumPlan.getRoamLine = function () {
      return roamLine;
    };
    ...
    CesiumPlan.deleteDraw = function (viewer, id) {
      if ((viewer.entities.getById(id) !== undefined)) {
        ...
        let ePlanConTemp = ePlanCon;
        for (let j = 0; j < ePlanConTemp.length; j++) {
          let ePlanConjson = ePlanConTemp[j].substring(0, ePlanConTemp[j].length - 1);
          let eplanObj = JSON.parse(ePlanConjson);
          if (eplanObj.id === id) {
            ePlanCon.splice(j, 1);
          }
          console.log(ePlanCon);
        }
      }
    };
