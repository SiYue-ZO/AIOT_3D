<template>
  <div id="stat-buildingRemoveTypeNum" class="stat-window stat-buildingRemoveTypeNum">
    <img class="stat-top" src="../assets/window/window_top.png"/>
    <div class="stat-middle stat-body">
      <div class="stat-content textshadow-p">
        <font-awesome-icon :icon="['fas', 'map-marked']" fixed-width></font-awesome-icon>
        国有集体地块数量占比
      </div>
      <div class="pop-chart" id="stat-buildingRemoveTypeNum-chart"></div>
    </div>
...
<script>
  import {countBuildingRemoveLandNum} from "../api/api";

  export default {
    name: "stat-buildingRemoveTypeNum",
    methods: {
      freshStat() {
        countBuildingRemoveLandNum().then(res => {
          ...
          let color = ['#ff3000', '#ffa800', '#ff5b00', '#00ffff', '#ffe000', '#00cfff', '#006ced'];
          let result = [];
          let data = [
          ...
          let rpaChart1 = this.$echarts.init(document.getElementById("stat-buildingRemoveTypeNum-chart"), 'light');
          let rpaOption1 = {
            title: {
              text: '国有集体地块数量占比',
              left: 'center',
