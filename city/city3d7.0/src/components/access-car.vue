<template>
  <div id="access-car-is-show">
    ...
    <el-table class="result-table" :data="tableDate.slice((currentPage-1)*pagesize,currentPage*pagesize)"
      ...
      <el-table-column label="牌照" prop="userId" width="70px" sortable></el-table-column>
      ...
      <el-table-column label="位置" prop="camName" width="85px" sortable></el-table-column>
    </el-table>
    <el-pagination
...
<script>
  import {getAccessCar, getVideoByMacUrl} from "../api/api";
        ...
        getAccessCar().then(res => {
          ...
          res.data.map(item => {
            switch (item.accessType) {
              case "0":
                item.accessType = "禁止车辆";
                break;
              case "1":
                item.accessType = "内部车辆";
                break;
              case "2":
                item.accessType = "外来车辆";
                break;
              default:
                break;
            }
