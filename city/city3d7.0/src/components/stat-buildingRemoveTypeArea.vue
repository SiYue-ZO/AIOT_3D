<template>
  <div id="stat-buildingRemoveTypeArea" class="stat-window stat-buildingRemoveTypeArea">
    <img class="stat-top" src="../assets/window/window_top.png"/>
    <div class="stat-middle stat-body">
      <div class="stat-content textshadow-p">
        <font-awesome-icon :icon="['fas', 'th-large']" fixed-width></font-awesome-icon>
        国有集体地块面积占比
      </div>
      <div class="pop-chart" id="stat-buildingRemoveTypeArea-chart"></div>
    </div>
...
<script>
    import {countBuildingRemoveLandArea} from "../api/api";

    export default {
        name: "stat-buildingRemoveTypeArea",
      methods: {
        freshStat() {
          countBuildingRemoveLandArea().then(res => {
            ...
            let color = ['#ffa800', '#00ffff', '#ffe000', '#00cfff', '#006ced',  '#ff5b00', '#ff3000'];
            let result = [];
            let data = [
              {name: "集体", value: res.data[0]},
              {name: "国有", value: res.data[1]},
            ];
            let splitV1 = 0;
            data.map((item, i) => {
            ...
            let rpaChart1 = this.$echarts.init(document.getElementById("stat-buildingRemoveTypeArea-chart"), 'light');
            let rpaOption1 = {
              title: {
                text: '国有集体地块面积占比',
                left: 'center',
