<template>
  <div id="access-man-list-is-show">
    <div class="pop-fai pop-fai-stop" v-on:click="stopAccessMan" title="停止清除">
  ...
  export default {
    ...
    created() {
      ...
      eventBus.$on("face_alarm", (message) => {
        if (Number(message.alarm.repoType) === 4) {
          this.startAccessMan(message);
        }
      });
    },
  }
