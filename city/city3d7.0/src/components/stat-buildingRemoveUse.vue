<template>
  <div id="stat-buildingRemoveUse" class="stat-window stat-buildingRemoveUse">
    <img class="stat-top" src="../assets/window/window_top.png"/>
    <div class="stat-middle stat-body">
      <div class="stat-content textshadow-p">
        <font-awesome-icon :icon="['fas', 'building']" fixed-width></font-awesome-icon>
        房屋用途占比
      </div>
      <div class="pop-chart" id="stat-buildingRemoveUse-chart"></div>
    </div>
...
<script>
  import {countBuildingRemoveUse} from "../api/api";

  export default {
    name: "stat-buildingRemoveUse",
    methods: {
      freshStat() {
        countBuildingRemoveUse().then(res => {
          ...
          let color = ['#00ffff', '#ffe000', '#00cfff', '#006ced', '#ffa800', '#ff5b00', '#ff3000'];
          let result = [];
          let data = [
            {name: "住宅", value: res.data[0]},
            {name: "商业", value: res.data[1]},
          ];
          let splitV1 = 0;
          data.map((item, i) => {
          ...
          let rpaChart1 = this.$echarts.init(document.getElementById("stat-buildingRemoveUse-chart"), 'light');
          let rpaOption1 = {
            title: {
              text: '房屋用途占比',
              left: 'center',
