let CoordinateTranslate = (function () {
	...
	_.prototype.CalculateCoordinates = function (point, azimuth, elevation, distance) {
		...
		let point1 = this.lonLat2WebMercator(point);
		let lnglat = null;
	...
	_.prototype.ConvertXYZToLLA = function (XYZCoor) {
		...
		let altitude = Math.sqrt(XYZCoor.x * XYZCoor.x + XYZCoor.y * XYZCoor.y) / Math.cos(latitude) - this.WGSa / Math.sqrt(1 - this.WGSe2 * Math.sin(latitude) * Math.sin(latitude));
