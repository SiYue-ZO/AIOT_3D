<template>
  <div v-bind:id="'popupDiv'+popId" class="mapPop"
       v-bind:ref="'rcaPop-'+levelNum" v-on:click="clickStick('popupDiv'+popId)">
...
<script>
  import realCompany from '@/components/real-company';
  let popzIndex = 0;
  export default {
    name: "pop-realCompanyArea",
    data() {
      return {
        popId: "",
        levelNum: 0,//层级
    ...
    methods: {
      showChart() {
        ...
        let rpaOption1 = {
          ...
          legend: {
            ...
            orient: 'vertical',
            x: 'left',
            data: ['人员密集场所', '企事业单位', '重点单位', '特种行业', '娱乐场所', '党政机关', '清真寺']
          },
          series: [
            {
              name: '单位类型',
      ...
      pop_in(){
        realCompany.methods.stepInNextLevel(this.popId);
      },
      pop_close() {
        let pop = this.$refs['rcaPop-' + this.levelNum];
        if (pop) {
