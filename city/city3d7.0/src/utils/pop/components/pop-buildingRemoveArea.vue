<template>
  <div v-bind:id="'popupDiv'+popId" class="mapPop"
       v-bind:ref="'braPop-'+levelNum" v-on:click="clickStick('popupDiv'+popId)">
...
<script>
  import cityBuilding from '@/components/city-building-remove';

  let popzIndex = 0;
  export default {
    name: "pop-buildingRemoveArea",
    data() {
      return {
        popId: "",
        levelNum: 0,//层级
    ...
    methods: {
      ...
      pop_in() {
        cityBuilding.methods.stepInNextLevel(this.popId);
      },
      pop_close() {
        let pop = this.$refs['braPop-' + this.levelNum];
        if (pop) {
