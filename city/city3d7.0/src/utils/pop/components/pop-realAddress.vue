<template>
  <div v-bind:id="'popupDiv'+popId" class="infoPop" v-bind:ref="'raPop-'+levelNum"
       v-on:click="clickStick('popupDiv'+popId)">
    <div class="infoPop-title">
      <div class="infoPop-name">地名地址</div>
...
<script>
  ...
  export default {
    ...
    methods: {
      pop_close() {
        let pop = this.$refs['raPop-' + this.levelNum];
        if (pop) {
