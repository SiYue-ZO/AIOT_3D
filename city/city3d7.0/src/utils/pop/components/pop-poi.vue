<template>
  <div v-bind:id="'popupDiv'+popId" style="box-shadow:0 0 15px #00d3e7" class="mapPop"
       v-bind:ref="'poiPop-'+levelNum" v-on:click="clickStick('popupDiv'+popId)">
...
<script>
  ...
  export default {
    ...
    methods: {
      pop_close() {
        viewer.entities.getById('pi' + this.popId).show = true;
        let pop = this.$refs['poiPop-' + this.levelNum];
        if (pop) {
