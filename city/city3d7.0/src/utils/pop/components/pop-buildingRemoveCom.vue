<template>
  <div v-bind:id="'popupDiv'+popId" class="mapPop"
       v-bind:ref="'brcPop-'+levelNum" v-on:click="clickStick('popupDiv'+popId)">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      pop_in() {
        cityBuilding.methods.stepInCommunity(this.popId);
      },
      pop_close() {
        let pop = this.$refs['brcPop-' + this.levelNum];
        if (pop) {
