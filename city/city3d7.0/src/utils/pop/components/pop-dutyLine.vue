<template>
  <div v-bind:id="'popupDiv'+popId" class="infoPop" v-bind:ref="'dlPop-'+levelNum"
       v-on:click="clickStick('popupDiv'+popId)">
    <div class="infoPop-title">
      <div class="infoPop-name">{{lName}}</div>
...
<script>
  ...
  export default {
    ...
    methods: {
      pop_close() {
        let pop = this.$refs['dlPop-' + this.levelNum];
        if (pop) {
